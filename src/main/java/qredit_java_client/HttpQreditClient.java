...
public class HttpQreditClient implements QreditClient {
  ...
  public void updatePeers() {
    ...
    qreditNetwork.getPeerSettings().parallelStream().forEach(host -> {
      try {
        String baseUrl =
            qreditNetwork.getHttpScheme() + "://" + host.getHostname() + ":" + host.getPort();
    ...
    qreditNetwork.getTrustedPeerSettings().stream()
        .forEach(trustedPeerSetting -> {
          ...
          allPeers.stream()
              .filter(p -> p.getIp().equals(trustedPeerSetting.getHostname()) && p.getPort()
                  .equals(trustedPeerSetting.getPort()))
              .findFirst()
  ...
  @Override
  public List<Transaction> getTransactionByRecipientAddress(String recipientAddress, Integer limit,
      Integer offset) {
    return restTemplate
        .exchange(
            getRandomTrustedPeerUrl() + "/api/transactions?orderBy=timestamp:desc" +
  ...
  @Override
  public String broadcastTransaction(String recipientId, Long satoshiAmount, String vendorField,
      String passphrase, Integer nodes) {
    Date beginEpoch;
    ...
    // sign transaction
    String senderPublicKey = BaseEncoding.base16().lowerCase()
        .encode(Crypto.getKeys(passphrase).getPubKey());
    ...
    targetPeers
        ...
        .forEach(peer -> {
          try {
            ...
            HttpEntity<CreateQreditTransactionsRequest> requestEntity = new HttpEntity<>(
                createQreditTransactionsRequest, headers);
            ...
            if (result.getBody().getTransactionIds() != null
                && result.getBody().getTransactionIds().size() > 0) {
              transactionIds.addAll(result.getBody().getTransactionIds());
            } else {
              log.info(
                  "Failed to broadcast transaction to node " + peer.getIp() + ":" + peer.getPort()
  ...
  private byte[] getBytes(CreateQreditTransactionRequest createQreditTransactionRequest,
